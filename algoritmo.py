...
def buscar_permutas_diretas(df, origem_user, destinos_user_list):
    ...
    destinos_user_norm = [normalizar_texto(d) for d in destinos_user_list if pd.notna(d) and str(d).strip()]
    ...
    for i, linha_b in df.iterrows():
        ...
        destinos_b = [str(d).strip() for d in destinos_b if pd.notna(d) and str(d).strip()]
...
def buscar_triangulacoes(df, origem_user, destinos_user_list):
    ...
    for i, linha_b in df.iterrows():
        ...
        for j, linha_c in df.iterrows():
            ...
            destinos_c = [str(d).strip() for d in destinos_c if pd.notna(d) and str(d).strip()]
    ...
    for i, linha_b in df.iterrows():
        ...
        for j, linha_c in df.iterrows():
            ...
            for k, linha_d in df.iterrows():
                ...
                destinos_d = [str(d).strip() for d in destinos_d if pd.notna(d) and str(d).strip()]
